  ...
  `
    ...
    "
        ...
        "
			>
				<li
					className={` ${currentPage === 1 ? 'opacity-25 cursor-not-allowed' : ''
						}`}
				>
					<button
						onClick={() => {
					...
					<li key={index} className="block !min-w-fit overflow">
						{typeof number === 'string' ? (
							...
							<button
								...
								className={`${buttonClassName} ${number === currentPage && '!bg-black !font-bold !text-white'
									}`}
							>
								{number}
							</button>
				...
				<li
					className={` ${currentPage === totalPages ? 'opacity-25 cursor-not-allowed' : ''
						}`}
				>
					<button
						onClick={() => {
